...
class ImageLogger(Callback):
    ...
    def log_images_and_audios(self, pl_module, batch):
        ...
        mel = Mel(x_res=image_shape[2], y_res=image_shape[3], hop_length=self.hop_length)
        ...
        for k in images:
            ...
            for _, image in enumerate(images[k]):
                audio = mel.image_to_audio(Image.fromarray(image, mode="RGB").convert("L") if channels == 3 else Image.fromarray(image[:, :, 0]))
...
class HFModelCheckpoint(ModelCheckpoint):
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        ...
        convert_ldm_to_hf_vae(ldm_checkpoint, self.ldm_config, self.hf_checkpoint)
...
if __name__ == "__main__":
    ...
    trainer = Trainer.from_argparse_args(
        ...
        callbacks=[
            ImageLogger(every=args.save_images_batches, hop_length=args.hop_length),
            HFModelCheckpoint(
                ldm_config=config,
