...
class AudioDiffusion:
    def __init__(
        ...
        sample_size = (self.pipe.unet.sample_size, self.pipe.unet.sample_size) if type(self.pipe.unet.sample_size) == int else self.pipe.unet.sample_size
        self.mel = Mel(
            x_res=sample_size[1],
            y_res=sample_size[0],
...
class AudioDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        steps = steps or 50 if isinstance(self.scheduler, DDIMScheduler) else 1000
        ...
        if audio_file is not None or raw_audio is not None:
            ...
            if hasattr(self, "vqvae"):
                input_images = self.vqvae.encode(input_images).latent_dist.sample(generator=generator)
            ...
            mask = self.scheduler.add_noise(
                torch.tensor(input_images[:, np.newaxis, :]),
                noise,
        ...
        images = list(
            map(lambda _: Image.fromarray(_[:, :, 0]), images) if images.shape[3] == 1 else map(lambda _: Image.fromarray(_, mode="RGB").convert("L"), images)
        )
    ...
    def encode(self, images: List[Image.Image], steps: int = 50) -> np.ndarray:
        ...
        self.scheduler.set_timesteps(steps)
        sample = np.array([np.frombuffer(image.tobytes(), dtype="uint8").reshape((1, image.height, image.width)) for image in images])
